...
function parseMarkdownLink(link) {
  const [, title, url] = /\[(.+)\]\((https?:\/\/[^\s]+)(?: "(.+)")?\)|(https?:\/\/[^\s]+)/gi.exec(link);
  return { title, url };
}
...
const sendCommitsMessage = async (ctx, commits, repository) => {
  const messages = [];
  commits.forEach((commit) => {
    messages.push(
      `:green_circle: +:${commit.added.length} :red_circle: -: ${commit.removed.length} :notepad_spiral: Modified files: ${commit.modified.length} \n Message: ${commit.message}`
    );
  });
  await sendDiscordMessage(
    ctx,
    messages.join('\n -------------- \n'),
    `${messages.length} commits pushed at ${repository.full_name}`,
    sender,
    0xff00b3,
    '',
    repository.html_url
...
integration.event.on('/:componentName/webhook/:eventType', async (ctx) => {
  const {
    data: { action, comment, pull_request, sender, issue, changes, commits, repository },
  } = ctx.req.body.data;
  ...
  if (commits) {
    await sendCommitsMessage(ctx, commits, repository);
    return;
  }

  if (comment) {
    await sendCommentMessage(ctx, comment, sender, image, element);
    return;
  }
  ...
  if (changes) {
    await sendDiscordMessage(
      ctx,
      `from ${changes.title.from} to ${element.title}`,
      `${elementType} description updated`,
      sender,
      0x00ffff,
      '',
      element.html_url
