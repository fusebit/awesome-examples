...
These configuration values should be set so that the user can click "Run" and immediately get the result of the example route.  This should typically include a `tenantId` value in the url path, but might also include appropriate payload data or alternate request types if `GET` is not appropriate. 
