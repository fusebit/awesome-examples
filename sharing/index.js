...
module.exports = async (ctx) => {
  switch (ctx.path.toLowerCase()) {
    ...
    default:
      return {
        body: Buffer.from('Select either /edit, /stackoverflow, or /integrationsFeed.json').toString('base64'),
        bodyEncoding: 'base64',
...
const renderEdit = (ctx) => {
  ...
  const integrationPath = `/account/${ctx.accountId}/subscription/${ctx.subscriptionId}/integration/${
    ctx.query.integrationId
  }/edit?${getForkQueryString(ctx)}`;
